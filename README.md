...
2. **Species fields** (see types in [`src/plantCatalog.ts`](src/plantCatalog.ts)):
   ...
   - **`cultivars`**: array of `{ "id", "name" }` (and optionally `defaultIconId`, `functions`, `layers`, `blooming`, `fruiting` to override species defaults). Cultivar **`id`** must be unique within that species.
